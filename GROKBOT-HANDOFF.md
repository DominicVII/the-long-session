...
**Build stamp (now): `20260921b`**

- `<meta name="onum-build" content="20260921b">`
- IIFE `STAMP = "20260921b"` (uses `history.replaceState` to set `?v=` — **never `location.replace`**, that froze boot)
- `src/lib/onum-build.ts` → `export const ONUM_BUILD = "20260921b"`
...
4. `?safe=1` is the only Simple-force. Default is **Best / fullDetail** (`20260921b`). Settings can still pick Simple.
...
`MODEST` (Dell / Edge / grok.me iframe / ≤8GB) still **throttles the loop**. As of `20260921b` it must **not** force Simple graphics. Visuals = Best. `?safe=1` is the hatch.
