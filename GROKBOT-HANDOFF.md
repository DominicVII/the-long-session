...
**Build stamp (now): `20260921a`**

- `<meta name="onum-build" content="20260921a">`
- IIFE `STAMP = "20260921a"` (uses `history.replaceState` to set `?v=` — **never `location.replace`**, that froze boot)
- `src/lib/onum-build.ts` → `export const ONUM_BUILD = "20260921a"`
- Wrapper iframe: `/session.html?v=${ONUM_BUILD}` in `src/routes/index.tsx`

If you change the HTML, bump the letter (`20260921b`, …) in **meta + STAMP + ONUM_BUILD together**, copy the body to all four filenames, commit, push `main`.
...
4. `?safe=1` is the only Simple-force. Default is **Best / fullDetail** (`20260921a`). Settings can still pick Simple.
...
`MODEST` (Dell / Edge / grok.me iframe / ≤8GB) still **throttles the loop**. As of `20260921a` it must **not** force Simple graphics. Visuals = Best. `?safe=1` is the hatch.
